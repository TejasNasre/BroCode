...
const Project = require("../model/addproject.model.js");

//Task Creation ----> Create
router.post("/add-project", async (req, res) => {
  try {
    const { title, description, skillsRequired, liveLink, email } = req.body;
    ...
    if (isUserExist) {
      const project = new Project({
        ...
        skillsRequired,
        liveLink,
        createdBy : isUserExist,
      });
      await project.save().then(() => res.status(200).json({ project : project }));
      isUserExist.project.push(project);
